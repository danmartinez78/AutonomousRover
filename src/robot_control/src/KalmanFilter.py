...
class KalmanFilter:
    ...
    def __init__(self, markers):
        ...
        self.Q_t = np.eye(3) * 1000
        self.R_t = np.eye(3) * 100
    ...
    def prediction(self, v, imu_meas):
        ...
        #print self.dt
        self.dtheta = float(imu_meas[3])*self.dt
        self.theta += self.dtheta
        #print(v*math.cos(self.theta))*self.dt
        self.dx = (v * math.cos(self.theta) * self.dt)
        self.dy = (v * math.sin(self.theta) * self.dt)
        #print self.dx,self.dy,self.dtheta
        self.x += self.dx
        self.y += self.dy
