...
class RobotSim(object):
        ...
        for r in range(self.__occupancy_map.shape[0]):
            for c in range(self.__occupancy_map.shape[1]):
                if self.__occupancy_map[r,c]:
                    ...
                    obs_iter+=1
        # Markers
        self.__markers = [ 0 for i in range(len(self.markers)) ]
        self.__markers_dir = [ 0 for i in range(len(self.markers)) ]
        for m in range(len(self.markers)):
            angle=(self.markers[m][2]/np.pi)*180.0
            R = np.array([[np.cos(self.markers[m][2]), -np.sin(self.markers[m][2])],
                          [np.sin(self.markers[m][2]), np.cos(self.markers[m][2])]])
            ...
            self.__markers[m] = patches.Rectangle(
                (self.markers[m][0] - rotated_offset[0]/2.,
                 self.markers[m][1] - rotated_offset[1]/2.),
                self.__marker_width,
            ...
            self.__markers_dir[m] = patches.Rectangle(
                (self.markers[m][0] - rotated_dir_offset[0]/2.,
                 self.markers[m][1] - rotated_dir_offset[1]/2.),
    ...
    def get_measurements(self):
        ...
        """
        curr_time = self.__frame_num*self.__dt
        if curr_time - self.last_meas_time < self.__MEAS_RATE or len(self.markers) == 0:
            return None
        self.last_meas_time = curr_time
        self.__visible_markers = [False for i in range(len(self.markers))]
        ...
        for i in range(len(self.markers)):
            ...
            if marker_i[2] > np.pi:
                marker_i[2]-=2*np.pi
            elif marker_i[2] < np.pi:
                marker_i[2]+=2*np.pi
            H_WT = self.__H(marker_i)#self.markers[i])
            ...
            if abs(marker_view_angle) < self.__view_half_angle and abs(theta_new) < np.pi/3 and x_new[0] < 2:
                ...
                meas_i = np.array([x_new[0],x_new[1], theta_new, self.markers[i][3], self.last_meas_time])
